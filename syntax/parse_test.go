...
def h():
	pass`,
		...
		{"f();\n",
			`(ExprStmt X=(CallExpr Fn=f))`},
		// from abseil-cpp repository
		{`genrule(
			"""
			Copy the file generated by //cmd/foo to a file named 'foo' 
			"""
			name = "genfoo",
			srcs = ["//cmd/foo"],
			outs = ["foo"],
		...
		`,
			`(ExprStmt X=(CallExpr Fn=f))`},
		// triple quote alone is OK
		{`r(
			"""
			"""
		)
			`,
			`(ExprStmt X=(CallExpr Fn=r Args=("\n\t\t\t")))`},
		// triple quote followed by binary expr seems problematic: failed: got identifier, want ','
		{`f(
			"""
			"""
			x = "y",
